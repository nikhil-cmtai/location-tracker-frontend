...
// Headers for export
const headers: { key: keyof HierarchyDisplay; label: string }[] = [
  { key: 'name', label: 'Hierarchy Name' },
...
export default function HierarchyPage() {
  ...
  const displayHierarchies: HierarchyDisplay[] = hierarchies.map((hierarchy) => ({
    _id: hierarchy._id,
    name: hierarchy.name,
    level: hierarchy.level,
  }));

  const filteredHierarchies = displayHierarchies.filter((hierarchy) =>
    Object.values(hierarchy).some((value) =>
  ...
  const currentHierarchies = filteredHierarchies.slice(startIndex, endIndex);
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] mb-6">
        <div className="p-4 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {/* Export Buttons */}
            <div className="flex items-center gap-2">
              <ExportButtons data={displayHierarchies} headers={headers} filename="hierarchies" allData={displayHierarchies} />
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 flex-1 justify-end flex-wrap">
            ...
            {/* Add New Button */}
            <Link
              href="/dashboard/otc/hierarchy/add"
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] overflow-hidden">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[var(--border-light)]">
              {status === 'loading' ? (
                ...
                currentHierarchies.map((hierarchy, index) => (
                  <tr key={hierarchy._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <Link
                        href={`/dashboard/otc/hierarchy/${hierarchy._id}`}
        ...
        <div className="px-6 py-4 border-t border-[var(--border-light)] flex items-center justify-between flex-wrap gap-4">
          <div className="text-sm text-[var(--text-secondary)]">
            Showing results {startIndex + 1} out of {filteredHierarchies.length}
          </div>
          <div className="flex items-center gap-2">
            <button
