...
import { fetchOwnerTypes } from '@/lib/redux/features/VehicleOwnerSlice';

interface OwnerTypeDisplay {
  _id: string;
  ownerType: string;
  description: string;
}
...
// Headers for export
const headers: { key: keyof OwnerTypeDisplay; label: string }[] = [
...
export default function VehicleOwnerPage() {
  ...
  const displayOwnerTypes: OwnerTypeDisplay[] = ownerTypes.map((owner) => ({
    _id: owner._id,
    ownerType: owner.ownerType,
    description: owner.description,
  }));

  const filteredOwnerTypes = displayOwnerTypes.filter((owner) =>
    Object.values(owner).some((value) =>
      value.toString().toLowerCase().includes(searchQuery.toLowerCase())
    )
  );

  const totalPages = Math.ceil(filteredOwnerTypes.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentOwnerTypes = filteredOwnerTypes.slice(startIndex, endIndex);
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] mb-6">
        <div className="p-4 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {/* Export Buttons */}
            <div className="flex items-center gap-2">
              <ExportButtons data={displayOwnerTypes} headers={headers} filename="vehicle_owner_types" allData={displayOwnerTypes} />
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 flex-1 justify-end flex-wrap">
            ...
            {/* Add New Owner Type Button */}
            <Link
              href="/dashboard/masters/vehicle-owner/add"
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] overflow-hidden">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[var(--border-light)]">
              {status === 'loading' ? (
                ...
                currentOwnerTypes.map((owner, index) => (
                  <tr key={owner._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <Link
                        href={`/dashboard/masters/vehicle-owner/${owner._id}`}
        ...
        <div className="px-6 py-4 border-t border-[var(--border-light)] flex items-center justify-between flex-wrap gap-4">
          <div className="text-sm text-[var(--text-secondary)]">
            Showing results {startIndex + 1} out of {filteredOwnerTypes.length}
          </div>
          <div className="flex items-center gap-2">
            <button
