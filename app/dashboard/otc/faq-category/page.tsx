...
import { fetchFaqCategories } from '@/lib/redux/features/faqCategorySlice';

interface FaqCategoryDisplay {
  _id: string;
  name: string;
  description: string;
}
...
// Headers for export
const headers: { key: keyof FaqCategoryDisplay; label: string }[] = [
...
export default function FaqCategoryPage() {
  ...
  const displayCategories: FaqCategoryDisplay[] = faqCategories.map((category) => ({
    _id: category._id,
    name: category.name,
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] mb-6">
        <div className="p-4 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {/* Export Buttons */}
            <div className="flex items-center gap-2">
              <ExportButtons data={displayCategories} headers={headers} filename="faq_categories" allData={displayCategories} />
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 flex-1 justify-end flex-wrap">
            ...
            {/* Add New Button */}
            <Link
              href="/dashboard/otc/faq-category/add"
                ...
                currentCategories.map((category, index) => (
                  <tr key={category._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <Link
                        href={`/dashboard/otc/faq-category/${category._id}`}
