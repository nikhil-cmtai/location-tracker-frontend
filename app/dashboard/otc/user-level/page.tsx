...
// Headers for export
const headers: { key: keyof UserLevelDisplay; label: string }[] = [
...
export default function UserLevelPage() {
  ...
  const displayUserLevels: UserLevelDisplay[] = userLevels.map((level) => ({
    _id: level._id,
  ...
  const filteredUserLevels = displayUserLevels.filter((level) =>
    Object.values(level).some((value) =>
      value.toString().toLowerCase().includes(searchQuery.toLowerCase())
    )
  );

  const totalPages = Math.ceil(filteredUserLevels.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentUserLevels = filteredUserLevels.slice(startIndex, endIndex);
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] mb-6">
        <div className="p-4 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {/* Export Buttons */}
            <div className="flex items-center gap-2">
              <ExportButtons data={displayUserLevels} headers={headers} filename="user_levels" allData={displayUserLevels} />
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 flex-1 justify-end flex-wrap">
            ...
            {/* Add New Button */}
            <Link
              href="/dashboard/otc/user-level/add"
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[var(--border-light)]">
              ...
              ) : currentUserLevels.length > 0 ? (
                currentUserLevels.map((level, index) => (
                  <tr key={level._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <Link
                        href={`/dashboard/otc/user-level/${level._id}`}
        ...
        <div className="px-6 py-4 border-t border-[var(--border-light)] flex items-center justify-between flex-wrap gap-4">
          <div className="text-sm text-[var(--text-secondary)]">
            Showing results {startIndex + 1} out of {filteredUserLevels.length}
          </div>
          <div className="flex items-center gap-2">
            <button
