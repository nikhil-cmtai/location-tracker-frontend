...
import { fetchSims } from '@/lib/redux/features/simSlice';
...
// Headers for export
const headers: { key: keyof SimDisplay; label: string }[] = [
  { key: 'imeiNumber', label: 'IMEI Number' },
  { key: 'iccid', label: 'ICCID' },
  { key: 'simProvider', label: 'SIM Provider' },
  { key: 'primeryMSISDN', label: 'Primary MSISDN' },
];
...
export default function SimPage() {
  ...
  const displaySims: SimDisplay[] = sims.map((sim) => ({
    _id: sim._id,
    imeiNumber: typeof sim.imeiNumber === 'object' ? sim.imeiNumber.imeiNumber?.toString() || 'N/A' : 'N/A',
    iccid: sim.iccid?.toString() || 'N/A',
    simProvider: typeof sim.sim === 'object' ? sim.sim.serviceProviderName || 'N/A' : 'N/A',
    primeryMSISDN: sim.primeryMSISDN || 'N/A',
  }));

  const filteredSims = displaySims.filter((sim) =>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] mb-6">
        <div className="p-4 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {/* Export Buttons */}
            <div className="flex items-center gap-2">
              <ExportButtons data={displaySims} headers={headers} filename="sims" allData={displaySims} />
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 flex-1 justify-end flex-wrap">
            ...
            {/* Add New Button */}
            <Link
              href="/dashboard/masters/sim/add"
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] overflow-hidden">
            ...
            <tbody className="divide-y divide-[var(--border-light)]">
              {status === 'loading' ? (
                ...
                currentSims.map((sim, index) => (
                  <tr key={sim._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <Link
                        href={`/dashboard/masters/sim/${sim._id}`}
        ...
        <div className="px-6 py-4 border-t border-[var(--border-light)] flex items-center justify-between flex-wrap gap-4">
          <div className="text-sm text-[var(--text-secondary)]">
            Showing results {startIndex + 1} out of {filteredSims.length}
          </div>
          <div className="flex items-center gap-2">
            <button
