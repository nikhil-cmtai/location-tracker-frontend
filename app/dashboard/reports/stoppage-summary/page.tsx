...
import { fetchStoppageSummary, clearStoppageSummary, StoppageEvent } from '@/lib/redux/features/stoppageSummarySlice';
...
// Headers for export
const headers: { key: keyof StoppageEvent; label: string }[] = [
  { key: 'vehicleNumber', label: 'Vehicle Number' },
  { key: 'imeiNumber', label: 'IMEI' },
  { key: 'serviceType', label: 'Service Type' },
  { key: 'ownerType', label: 'Owner Type' },
  { key: 'totalStopDuration', label: 'Total Stop Duration' },
  { key: 'stopEventCount', label: 'Stop Event Count' },
];

export default function StoppageSummaryPage() {
  const dispatch = useDispatch<AppDispatch>();
  const { reportData, status } = useSelector((state: RootState) => state.stoppageSummary);
    ...
    <div className="p-6 bg-[var(--content-bg)]">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] mb-6 p-6">
        <h2 className="text-xl font-bold text-[var(--text-primary)] mb-6">
          Filter Stoppage Summary Report
        </h2>
      ...
      {reportData.length > 0 ? (
            ...
            <table className="w-full">
              <thead className="bg-[var(--navy-dark)] text-white">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-semibold">Total Stop Duration</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold">Stop Event Count</th>
                </tr>
              </thead>
                      ...
                      <>
                        <tr key={item.vehicleNumber} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-sm text-[var(--text-primary)] font-semibold">
                            {item.totalStopDuration || 'N/A'}
                          </td>
                          <td className="px-6 py-4 text-sm text-[var(--text-primary)]">
                            <span className="px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700">
                              {item.stopEventCount || 0}
                            </span>
                          </td>
                        </tr>
                                ...
                                <table className="w-full">
                                  <thead className="bg-gray-100">
                                    <tr>
                                      ...
                                      <th className="px-4 py-2 text-left text-xs font-semibold text-[var(--text-primary)]">Stop Start Time</th>
                                      <th className="px-4 py-2 text-left text-xs font-semibold text-[var(--text-primary)]">Stop End Time</th>
                                  ...
                                  <tbody>
                                    {item.details.map((detail, detailIndex) => (
                                      <tr key={detailIndex} className="border-b border-gray-200">
                                        ...
                                        <td className="px-4 py-2 text-xs text-[var(--text-primary)]">{detail.stopStartTime}</td>
                                        <td className="px-4 py-2 text-xs text-[var(--text-primary)]">{detail.stopEndTime}</td>
                                        <td className="px-4 py-2 text-xs text-[var(--text-primary)]">{detail.totalStopDuration}</td>
                                        <td className="px-4 py-2 text-xs text-[var(--text-primary)]">{detail.stopLocation || 'N/A'}</td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] p-12 text-center">
          <div className="text-[var(--text-secondary)]">
            {status === 'loading' ? (
              <div className="text-lg">Loading stoppage summary...</div>
            ) : (
              <div>
                <p className="text-lg mb-2">No stoppage summary data available</p>
                <p className="text-sm">Please select filters and click Generate Report to view stoppage summary</p>
              </div>
            )}
          </div>
        </div>
