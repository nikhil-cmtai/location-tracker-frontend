...
import { fetchDetailedStoppageSummary, clearDetailedStoppageSummary, DetailedStoppageSummary } from '@/lib/redux/features/detailedStoppageSlice';
...
// Headers for export
const headers: { key: keyof DetailedStoppageSummary; label: string }[] = [
  { key: 'sNo', label: 'S.No' },
...
export default function DetailedStoppagePage() {
  const dispatch = useDispatch<AppDispatch>();
  const { reportData, status } = useSelector((state: RootState) => state.detailedStoppage);
  ...
  const handleGenerate = () => {
    ...
    dispatch(fetchDetailedStoppageSummary(params));
  ...
  const handleClear = () => {
    dispatch(clearDetailedStoppageSummary());
    setSelectedVehicle('');
    setVehicleSearch('');
    setIsVehicleDropdownOpen(false);
    setStartDate(null);
    ...
    <div className="p-6 bg-[var(--content-bg)]">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] mb-6 p-6">
        <h2 className="text-xl font-bold text-[var(--text-primary)] mb-6">
          Filter Detailed Stoppage Report
        </h2>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-[var(--border-light)] mb-6">
          <div className="p-4 flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              {/* Export Buttons */}
              <div className="flex items-center gap-2">
                <ExportButtons data={currentData} headers={headers} filename="detailed-stoppage" allData={reportData} />
              </div>
            </div>
                        ...
                        <tr>
                          <td colSpan={12} className="px-6 py-4 bg-gray-50">
                            <div className="space-y-2">
                              <h4 className="font-semibold text-[var(--text-primary)] mb-3">Stoppage Details:</h4>
                              <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                  <thead className="bg-gray-100">
                                    <tr>
                                      <th className="px-4 py-2 text-left">Vehicle Number</th>
                                      <th className="px-4 py-2 text-left">Stop Start Time</th>
                                      <th className="px-4 py-2 text-left">Stop End Time</th>
                                      <th className="px-4 py-2 text-left">Total Stop Duration</th>
                                      <th className="px-4 py-2 text-left">Stop Location</th>
                                    </tr>
                                  </thead>
                                  <tbody className="divide-y divide-gray-200">
                                    {item.details.map((detail, detailIndex) => (
                                      <tr key={detailIndex} className="hover:bg-gray-100">
                                        ...
                                        <td className="px-4 py-2">{detail.stopStartTime}</td>
                                        <td className="px-4 py-2">{detail.stopEndTime}</td>
                                        <td className="px-4 py-2">{detail.totalStopDuration}</td>
                                        <td className="px-4 py-2">{detail.stopLocation}</td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
